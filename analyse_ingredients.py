...
stoplist = ['of', 'and', 'or', 'zo', 'c', 'in', 'f', 'to', 'as', 'to', 'one', 'beaten', 'weight', 'oz', 'white', 'dry', 'raw', 'hot', 'old', 'fresh', 'de', 'so', 'all', 'me', 'can', 'round', 'fine', 'extra','sauce','pack','cut','fluid','thin','pot','ounces','for','tin','if','mix','cube','fat','into','the','ice','up','cans','quart','quarts','finely','stock','cubes', 'ground','piece','pieces','baking','leave','thick','more','bag','thinly','green','pan', 'taste', 'skin', 'each', 'cup', 'pinches', 'half', 'with', 'slice','cold', 'spice','frozen', 'whole','chile', 'plus', 'dice', 'top', 'chunks', 'syrup', 'bar','package', 'small','stick', 'bone', 'not','shortening', 'use','crust', 'size', 'off', 'seasoning', 'free', 'lb', 'from','amp','plain', 'pod','strips', 'total', 'large', 'wedge', 'leg', 'ripe','lightly', 'dish','optional', 'fruit', 'stem', 'peel', 'stalk', 'shell', 'flower', 'torn', 'tub', 'fillet', 'breast', 'meat','roughly','garni', 'length', 'serve', 'head','long', 'only','room', 'temperature','well','rounds', 'brown', 'bunch', 'meal', 'squash', 'beet', 'yellow', 'coarsely', 'ones', 'dates', 'box', 'heart', u'\u215e', 'loaf', 'rings', 'less', 'liquid', 'ready', 'container', 'blend', 'then','cod','over', 'lean', 'sheet', 'solids', 'hard', 'square', 'spoon', 'ends', 'bottle', 'wide', 'halves', 'bits','diameter','recipe','ball','extract', 'thighs','packet','on', 'but', 'pin','see', 'out', 'vegetable', 'edges','fillets', 'sticks', 'halfandhalf', 'grain', 'stems', 'rib', 'cups', 'cola','hearts','broken','bulb','such as','split','dressing','rind','breasts', 'medium','lengths','lengthwise','jars','pit','greens','freshly','liver', 'chops','smoke', 'squares', 'oven', 'flakes', 'neck', 'balls', 'wings', 'sheets', 'thickly', 'serving', 'pns', 'legs', 'heads', 'tops', 'tails', 'tins', 'deep', 'dog', 'tips', 'crusts', 'soy','bars', 'thickness', 'be','slices','heat','you','soup','light','bun','parts','note','tip','jus','bones','loin','roast','cob','lengthways','mince','bags','pound','block','clean','preferably','until','coating','tsp','bes','pea','soft','left','like','prefer','below','palm','base','slightly','cn','bulbs','triple sec','core','about','rest','segments','skins','using','organic','filling','pans','bitesize','cooking','loosely','person','excess','fat','spread','spoons','both','bake','colour','sole','dogs','fingers','very','twist','flowers','sides','when','intact','fork','is','fine','decorate','content','range','good','spicy','portion','above','snow','at','left','bottles','per',u'\u2153',u'\u2013',u'\u215c','coins','approx','this', 'it','a','glass','four','case','some','no','flat','rub','add','bought','blade','made','want','go','recipe','tough','ounce','full','make','just','i','time','than','avoid','theyre','self','any','its','dinner','best','r','easy','steps','right','by','halfnhalf','t','lo','ui','sp','jar','pie','powder','gin','rum','cream','juice','seeds','leaf','leaves','nut','nuts','peas','paste', 'fish','fruits', 'ale', 'jam','salsa','cake','black','soda','vine','un','sour','bell','cke','berries','ish','chill','dark','marg','ange','pk','ear','cap','same','golden','wedges','pine','son','masa','pre','swiss','end','anis','hand','ring','single','creme','pinch','wrap','pn','alu','veg','lid','side','hm','semi','frying','allpurpose','paper','brick','skinless','circle','bowl','your','boneless','st','alt','choc','bre','opt','temp','wal','oat','double','lot','root','etc','hs','season','tea','eel','warm','wise','tart','port','bis','cent','rack','tail','thai','die','two','jr','caps','kept','msg','firmly','layer','s','nd','tu','fill','type','them','that','oval','inch','feet','star','once','coin','dont','still','sure','mine','tbsp','regular','through','mixer','bottom','mixture','preserves','blue','baby','start','pots','again','will','do','ideal','low','overnight','smart','possible','middle']
        ...
        while startswith(name, stoplist):
            for word in stoplist:
                if name.startswith(word+' ') or name == word:
                    name = name.replace(word, '').strip()#replaces only first occurrence
        if len(name) == 0:
            continue
        #print ing
        for cnt in counts:
...
def main():
    ...
    mappings = {}
    for what in cntr.most_common(len(unique_ingredients)/3): # we need a constant here because otherwise we get rubbish ingredient as the most popular one...
        n1 = what [0]
