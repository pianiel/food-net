...
select s.id as id, name as label, count from 
(select ingredient_id as id, count(ingredient_id) as count 
  from recipes_ingredients 
  group by ingredient_id) as s 
join ingredients as i on s.id = i.id where s.count > 3;

--edges using only vertices from the above query
select i1.name, i2.name, source, target, weight from 
( select distinct  a.ingredient_id as source,  b.ingredient_id as target,  count(a.ingredient_id) as weight
  ...
  join recipes_ingredients as b  using (recipe_id) 
  where a.ingredient_id < b.ingredient_id 
  and a.ingredient_id in 
   (select s.id from 
   	(select ingredient_id as id, count(ingredient_id) as count 
   		from recipes_ingredients group by ingredient_id) as s 
  ...
  and b.ingredient_id in 
    (select s.id from 
   	  (select ingredient_id as id, count(ingredient_id) as count 
   		from recipes_ingredients group by ingredient_id) as s 
   	  join ingredients on s.id = ingredients.id where count > 3) 
    group by source, target ) as s
join ingredients as i1 
on source = i1.id
join ingredients as i2
on target = i2.id
where s.weight > 2 order by weight desc, source, target;
