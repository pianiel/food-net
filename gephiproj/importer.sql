...
(select s.id from (select ingredient_id as id, count(ingredient_id) as count from recipes_ingredients group by ingredient_id) as s join ingredients on s.id = ingredients.id where count > 3) 
and b.ingredient_id in 
(select s.id from (select ingredient_id as id, count(ingredient_id) as count from recipes_ingredients group by ingredient_id) as s join ingredients on s.id = ingredients.id where count > 3) 
group by source, target ) as s where s.weight > 2 order by weight desc, source, target;
